...
namespace sql {
    ...
    class Param {
    public:
        explicit Param(std::string_view &param) : _param(param) {}

    public:
        std::string operator()() const { return param(); }

        [[nodiscard]] const std::string &param() const { return _param; }
    ...
    template<>
    inline std::string to_value<func>(const func &data) {
        return data.str();
    }
    ...
    inline std::string str_join(const std::vector<std::string> &v, const std::string &delim) {
        ...
        if (it != std::end(v)) {
            ret = *it;
            while (++it != std::end(v)) ret += delim + *it;
        }
        return ret;
    }

    template<typename T>
    std::string str_val_join(const std::vector<T> &v, const std::string &delim) {
        std::string ret;
        auto it = std::begin(v);
        if (it != std::end(v)) {
            ret = to_value<T>(*it);
    ...
    class column {
    public:
        explicit column(std::string_view column) {
            _cond = column;
        }
        ...
        column &like(std::string_view like) {
            _cond.append(" LIKE ('");
            _cond.append(like);
            _cond.append("%')");
    ...
    public:
        ...
        const std::string &str() override {
            ...
            _sql.append(str_join(_select_columns, ", "));
            _sql.append(" from ");
            ...
            if (!_join_on_condition.empty()) {
                _sql.append(" on ");
                _sql.append(str_join(_join_on_condition, ", "));
            }
            if (!_where_condition.empty()) {
                _sql.append(" where ");
                _sql.append(str_join(_where_condition, " and "));
            }
            if (!_groupby_columns.empty()) {
                _sql.append(" group by ");
                _sql.append(str_join(_groupby_columns, ", "));
            }
            if (!_having_condition.empty()) {
                _sql.append(" having ");
                _sql.append(str_join(_having_condition, " and "));
            }
            if (!_order_by.empty()) {
    ...
    public:
        ...
        template<typename... Args>
        InsertModel &returning(const std::string &r, Args &&... returns) {
            _returning.push_back(r);
            returning(returns...);
            return *this;
        }

        // for recursion
        InsertModel &returning() {
            return *this;
        }

        InsertModel &returning(const std::string &c) {
            _returning.push_back(c);
            return *this;
        }
        ...
        const std::string &str() override {
            ...
            _sql.append(_table_name);
            _sql.append("(");
            _sql.append(str_join(_columns, ", "));
            _sql.append(")");
            _sql.append(" values(");
            _sql.append(str_join(_values, ", "));
            _sql.append(")");

            if (!_returning.empty()) {
                _sql.append(" returning ");
                _sql.append(str_join(_returning, ", "));
            }
            return _sql;
        }
    ...
    public:
        ...
        virtual const std::string &str() override {
            ...
            _sql.append(_table_name);
            _sql.append(" set ");
            _sql.append(str_join(_set_columns, ", "));
